import { getPostByRoute, getWordCount } from "@/lib/notion";
...
interface RoutePageProps {
  params: Promise<{ firstSlash: string; postTitle: string }>;
}
...
export async function generateMetadata(
  { params }: RoutePageProps,
  parent: ResolvingMetadata
): Promise<Metadata> {
  const { firstSlash, postTitle } = await params;
  const post = await getPostByRoute(firstSlash, postTitle);
...
export default async function RoutePage({ params }: RoutePageProps) {
  ...
  const jsonLd = {
    ...
    publisher: {
      ...
      logo: {
        "@type": "ImageObject",
        url: `/blog/logos/climatefair-logo.png`,
      },
    },
    mainEntityOfPage: {
      "@type": "WebPage",
